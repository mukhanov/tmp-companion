...
**A second unit says the muted state is NOT a firmware default.** The development unit (**fw 1.8.45**, `probe --capture-input 6` / `--fw`, 2026-08-24) reports `usb3`/`usb4` both `muteActive: false`, `preEnabled: true`, `faderLevel: 1.0` — the clean state — and negotiates the full 4-channel capture with an actual dry lane on USB-Out 3. So the reporting unit's muted USB 3/4 was its own setting (or something that unit's history did to it), not something fw 1.8.58 ships. Do not "fix" this by assuming new firmware mutes the dry sends.
