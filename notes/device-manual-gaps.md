...
**A second unit says the muted state is NOT a firmware default.** The development unit (`probe --capture-input 6`, 2026-08-24) reports `usb3`/`usb4` both `muteActive: false`, `preEnabled: true`, `faderLevel: 1.0` — the clean state — and negotiates the full 4-channel capture with a real dry lane on USB-Out 3. So the reporting unit's muted USB 3/4 was its own setting (or something that unit's history did to it), not something fw 1.8.58 ships. Do not "fix" this by assuming new firmware mutes the dry sends.

**Floor caveat, same run.** That unit's idle dry lane peaks at −74.6 dBFS (ch0/ch1 at −71.5), i.e. ABOVE `DRY_AUDIBLE_PEAK` (1e-4 = −80 dBFS). A not-playing take there is therefore not classed as silent, skips `silent_dry_diagnosis` entirely, and surfaces as the terser "captured signal too quiet to measure" from the non-finite-LUFS gate instead (confirmed: the probe reads `lufs -inf` on all four lanes). Safe, but the richer diagnosis does not reach a unit whose noise floor clears the threshold — worth revisiting with peaks from more units before retuning the constant.
